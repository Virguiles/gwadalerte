...
export function useMeteoForecast(options: UseForecastOptions = {}): UseForecastReturn {
  ...
  /*
   * Chargement initial. Dette assumée, comme dans `useMeteoData` : sur cache
   * chaud, `fetchForecast` pose l'état sans passer par le réseau, donc de
   * façon synchrone. À traiter avec la reprise du cache des prévisions.
   */
  useEffect(() => {
    if (autoFetch) {
...
export function usePreloadForecasts(codeZones: string[]) {
  ...
  // Même dette que ci-dessus : `preload` marque les zones déjà vues dans un
  // état, et le fait tout de suite pour celles que le cache couvre déjà.
  useEffect(() => {
